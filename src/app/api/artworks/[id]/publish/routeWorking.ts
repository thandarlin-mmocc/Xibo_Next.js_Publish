...
export async function POST(
  ...
  try {
    ...
    if (fileErr) {
      // If already exists, search and reuse existing media
      if (
        String(fileErr)
          .toLowerCase()
          .includes("already own media with this name")
      ) {
        const existingMediaId = await findMediaIdByName(mediaName);
        if (!existingMediaId) {
          return NextResponse.json(
            {
              error: "Media exists but could not be found by name",
              mediaName,
              fileErr,
              xiboUpload,
            },
        ...
        // ✅ assign using existingMediaId (NOT mediaId)
        try {
          await assignToPlaylist(playlistId, existingMediaId, 10);
        } catch (e: any) {
          return NextResponse.json(
            {
        ...
        await prisma.artwork.update({
          where: { id: artworkId },
          data: { xiboMediaId: existingMediaId },
        });
      ...
      return NextResponse.json(
        { error: "Xibo upload failed", details: fileErr, xiboUpload },
        { status: 400 }
      );
    }
